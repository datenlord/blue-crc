...
def run_test(conf_file_path):
    ...
    print(
        f"Start Tests on: crc_width={crc_width} axi_keep_width={axi_keep_width} crc_mode={crc_mode}"
    )
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        run_test(conf_file_path)
    else:
        config_dir = os.path.abspath("../../scripts/config")
        for root, dirs, files in os.walk(config_dir):
            for file_name in files:
                conf_file_path = os.path.join(root, file_name)
                result = run_test(conf_file_path)
                info = f"Testbench Failed on the configuration of {file_name}."
